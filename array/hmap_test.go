...
func TestHmap(t *testing.T) {
	...
	t.Run("Hmap", func(t *testing.T) {
		...
		assert.Panics(func() { hmap.Get(0) }, "algos.array.(hmap).Get():  Is not panics when key is not found")
		assert.Panics(func() { hmap.Get(3) }, "algos.array.(hmap).Get():  Is not panics when key is not found")
		assert.Panics(func() { hmap.Del(3) }, "algos.array.(hmap).Del():  Is not panics when key is not found")
		hmap.Del(1)
		assert.Equal(hmap.IsKey(1), false)
		assert.Equal(hmap.IsEmpty(), false)
